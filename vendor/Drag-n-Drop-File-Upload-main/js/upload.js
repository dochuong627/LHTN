...
fileInput.onchange = ({target})=>{
  ...
  if (file.length === 1){
    ...
    if (!allowed_EXT.exec(file[0].name)) {
      showToast('Vì lý do bảo mật, vui lòng nén văn kiện lại (zip)', 'red');
    }
    else{
    ...
    showToast('Vì lý do bảo mật, vui lòng nén văn kiện lại (zip)', 'blue');
  }
}

dropArea.addEventListener("dragover", (event)=>{
  event.preventDefault();
  dragText.textContent = "Thả ra để thêm văn kiện";
...
dropArea.addEventListener("dragleave", ()=>{
  dragText.textContent = "Kéo thả văn kiện, thông báo vào đây";
...
function drop_Upload(drop_files){
  ...
  ajax_request.upload.addEventListener("progress", ({loaded, total}) =>{
    ...
    let progressHTML = `<li class="row">
                          ...
                          <div class="content">
                            <div class="details">
                              <span class="name">${dro_file} • đang tải</span>
    ...
    if(loaded == total){
      ...
      let uploadedHTML = `<li class="row">
                            <div class="content upload">
                              <i class="fas fa-file-alt"></i>
                              <div class="details">
                                <span class="name">${dro_file} • tải xong</span>
...
function uploadFile(name){
  ...
  xhr.upload.addEventListener("progress", ({loaded, total}) =>{
    ...
    let progressHTML = `<li class="row">
                          ...
                          <div class="content">
                            <div class="details">
                              <span class="name">${name} • đang tải</span>
                              <span class="percent">${fileLoaded}%</span>
                            </div>
                            <div class="progress-bar">
                              <div class="progress" style="width: ${fileLoaded}%"></div>
    ...
    if(loaded == total){
      ...
      let uploadedHTML = `<li class="row">
                            <div class="content upload">
                              <i class="fas fa-file-alt"></i>
                              <div class="details">
                                <span class="name">${name} • tải xong</span>
                                <span class="size">${fileSize}</span>
                              </div>
                            </div>
                            <i class="fas fa-check"></i>
